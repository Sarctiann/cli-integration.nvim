...
- 2026-05-26: Add OSC 52 sanitizer to prevent garbage text in TUI input: `setup_terminal_sanitizer(buf, job_id)` in window.lua uses `TextChanged` autocmd with 150ms debounce to detect OSC 52 sequences (`ESC ] 52 ;`) in terminal buffer and forces TUI redraw via Device Status Report (`ESC [ c`)
