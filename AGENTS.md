...
- 2026-05-26: Fix mouse copy in fullwidth mode: added `resize_pty()` helper that calls `vim.fn.jobresize()` after any window dimension change (fullwidth toggle, sidebar restore, editor resize) to send SIGWINCH so TUI apps update their internal size and mouse coordinates work correctly
- 2026-05-26: Fix vsplit phantom on fullwidth toggle: replaced `nvim_win_set_width(sidebar_win, 0)` with `nvim_win_close(sidebar_win, true)` in `update_sidebar_geometry()` so the vsplit is properly closed instead of collapsing to a 0-width phantom split; restore path already handles missing vsplit via `create_sidebar_layout()` fallback
...
- 2026-06-04: Fixed fullscreen float covering statusline: height formula changed from `lines - cmdheight - 2` to `lines - cmdheight - 3` in all 3 code paths (`update_sidebar_geometry`, `update_float_geometry`, `resize_sidebars`). The `-3` accounts for the single border's bottom row (`row = height + 1`) and ensures it does not reach the statusline (`row = lines - cmdheight - 1`). Enhanced debug logging across all window events with detailed dimension data. Updated docs (module-window.md, window-system-architecture.md, common-pitfalls.md).
- 2026-06-06: Fixed sidebar PTY resize losing height on file open/close: removed bounce check in `resize_sidebars()` deferred callback that skipped PTY resize when layout changed between `WinResized` and the `vim.schedule` callback. Now always resizes PTY to current (final) window dimensions, ensuring TUI apps always see correct dimensions after layout redistribution.
- 2026-06-06: Fixed sidebar height oscillation on file open/close: added `winfixheight = true` to sidebar window options in `apply_sidebar_win_opts()` (window.lua line 60). Without it, Neovim redistributed the sidebar's height when opening/closing horizontal splits in other windows, causing the sidebar to lose or gain 1 line unpredictably. The sidebar already had `winfixwidth` — `winfixheight` is the vertical counterpart.
