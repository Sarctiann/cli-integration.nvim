...
- 2026-05-26: Revert vsplit close back to width=0 collapse: `nvim_win_close` caused visual artifacts (dashes/guiones) because recreating the vsplit from scratch reinitializes window/buffer state; reverting to `nvim_win_set_width(sidebar_win, 0)` keeps the vsplit alive and avoids reinitialization
