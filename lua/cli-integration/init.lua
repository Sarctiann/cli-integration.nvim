...
function M.setup(user_config)
	...
	local command_complete = function(_, cmd_line, cursor_pos)
		...
		if #args > 0 and (args[1] == "CLIIntegration" or args[1] == "CIcmd") then
			table.remove(args, 1)
		end

		if #args == 0 then
			return { "open_cwd", "open_root", "dbg_print" }
		end
	...
	vim.api.nvim_create_user_command("CLIIntegration", command_handler, vim.tbl_extend("force", command_opts, {
		desc = "Open CLI Integration",
	}))
	vim.api.nvim_create_user_command("CIcmd", command_handler, vim.tbl_extend("force", command_opts, {
		desc = "Alias for CLIIntegration",
	}))
